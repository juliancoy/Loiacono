...
QString buildShaderSource(int signalLength)
{
    ...
    const QString buffers =
        ...
        "layout(std430, set = 0, binding = 5) buffer offset_buf { readonly uint offset[16]; };\n"
        "layout(push_constant) uniform Params { float leakiness; } params;\n";

    source.replace("uniform float leakiness;", "");
    source.replace("leakiness", "params.leakiness");
...
public:
                   ...
                   const std::vector<int>& windowLens)
    {
        ...
        if (!ensurePipeline(signalLength)) return false;
    ...
    bool compute(const std::vector<float>& ring, unsigned int offset, float leakiness, std::vector<float>& outSpectrum)
    {
        ...
        offsets[0] = offset;
        std::memcpy(offsetBuffer_.mapped, offsets.data(), sizeof(offsets));
...
private:
    ...
    bool ensurePipeline(int signalLength)
    {
        if (pipeline_ != VK_NULL_HANDLE && signalLength == signalLength_) return true;

        destroyPipeline();

        QByteArray spirv = compileShader(signalLength);
    ...
    bool ensureBuffers(int signalLength, int numBins)
    {
        ...
        if (!createBuffer(16 * sizeof(uint32_t), offsetBuffer_)) return false;

        std::array<VkWriteDescriptorSet, 6> writes{};
        std::array<VkDescriptorBufferInfo, 6> infos{};
        const Buffer* buffers[] = {&signalBuffer_, &outputBuffer_, &freqBuffer_, &normBuffer_, &windowBuffer_, &offsetBuffer_};
    ...
    QByteArray compileShader(int signalLength)
    {
        const QString source = buildShaderSource(signalLength);
        if (source.isEmpty()) return {};

        QTemporaryDir dir;
        if (!dir.isValid()) return {};

        const QString srcPath = dir.filePath("loiacono_vulkan.comp");
        const QString spvPath = dir.filePath("loiacono_vulkan.spv");
        ...
        QProcess proc;
        proc.start("glslangValidator", {"-V", srcPath, "-o", spvPath});
